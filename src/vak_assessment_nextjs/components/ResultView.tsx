...
export default function ResultView({ scores, dominantType, onRestart }: ResultViewProps) {
  ...
  // メールプレビュー表示時に該当タイプへスクロール
  useEffect(() => {
    if (showEmailPreview) {
      setTimeout(() => {
        const element = document.getElementById(`type-${dominantType}`);
        if (element) {
          element.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
      }, 100);
    }
  }, [showEmailPreview, dominantType]);
                ...
                {index === 0 ? (
                  ...
                  >
                    ...
                    <pre className="whitespace-pre-wrap font-sans text-sm leading-relaxed text-gray-800">
                      {generateHeader(name)}{generateTypeEmail(type)}
                    </pre>
                  </div>
                ) : (
                  // その他のタイプ例
